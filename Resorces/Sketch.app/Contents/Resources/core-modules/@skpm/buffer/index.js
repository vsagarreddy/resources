...
function isNSData(obj) {
  return (
    !!obj &&
    typeof obj.isKindOfClass === "function" &&
...
function Buffer(arg, encodingOrOffset, length) {
  ...
  if (typeof arg === "number") {
    if (util.isString(encodingOrOffset)) {
      throw new TypeError(
...
function fromString(string, encoding) {
  if (!util.isString(encoding) || encoding === "") {
    encoding = "utf8";
  }
...
function fromNSData(nsdata, encodingOrOffset, length) {
  ...
  const data = String(
    NSString.alloc().initWithData_encoding(nsdata, NSISOLatin1StringEncoding)
  );
...
function bidirectionalIndexOf(buffer, val, byteOffset, encoding, dir) {
  ...
  if (util.isString(byteOffset)) {
    encoding = String(byteOffset);
    byteOffset = 0;
...
Buffer.prototype.write = function write(string, offset, length, encoding) {
  ...
  } else if (length === undefined && util.isString(offset)) {
...
Buffer.prototype.fill = function fill(val, start, end, encoding) {
  ...
  if (util.isString(val)) {
    ...
    if (util.isString(start)) {
      encoding = String(start);
      start = 0;
      end = this.length;
    } else if (util.isString(end)) {
      encoding = String(end);
      end = this.length;
    }
    ...
    if (util.isString(encoding) && !Buffer.isEncoding(encoding)) {
      throw new TypeError("Unknown encoding: " + encoding);
    }
    if (val.length === 1) {
